...
class User < ApplicationRecord
  ...
  def self.from_omniauth(auth)
    ...
    # 5. Se o Google enviou uma foto de perfil e o usuário não tem uma, tentamos anexar
    if auth.info.image.present? && !user.avatar.attached?
      begin
        ...
        Rails.logger.error "Ocorreu um erro ao baixar a foto do Google: #{e.message}"
      end
    end
